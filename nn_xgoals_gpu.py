...
model = XGoalNeuralNetwork(input_dim).to(device) # model now moved to gpu if avail
# model = GoalNetwork()
...
with torch.no_grad(): # basically turn off back propagation
    ...
    y_pred_logits = model(torch.tensor(X_predict_postprocess_df.to_numpy().to(device),dtype=torch.float32))
