...
fn_predict = shots_dir + 'shots_2024_2025.csv'
...
epochs = 5 # early stopping will be enabled so this is an upper limit
...
BATCH_SIZE = 128 # batches per epoch = sample size / batch_size
...
for epoch in range(epochs):
    ...
    for X_batch, y_batch in train_loader:
        ...
        y_train_calc = model(X_batch)

        # measure loss for training data, y_train_tensor.unsqueeze(1).float()
        loss_train = criterion(y_train_calc, y_batch.unsqueeze(1).float()) # to fix the shape
        # loss_train = criterion(y_train_calc, y_batch.view(-1, 1).float()) # to fix the shape
        # loss_train = criterion(y_train_calc, y_batch) # this is the tensor that tracks gradients
        ...
        # accumulate batch loss, loss_train.item() is the average loss for the batch so x by size
        epoch_train_loss += loss_train.item() * X_batch.size(0) # sum loss over batch
    ...
    with torch.no_grad(): # disable gradient calculation during validation
        for X_batch_val,y_batch_val in val_loader:
            y_val_calc = model(X_batch_val)
            loss_val = criterion(y_val_calc,y_batch_val) # calculate validation loss
    ...
    if (epoch) % 5 == 0 or epoch == (epochs - 1):  # print loss every 50 epochs
        current_lr = optimizer.param_groups[0]['lr']
        print(f"Epoch {epoch:4d} | Training Loss: {epoch_train_loss:.4f} | Validation Loss: {epoch_val_loss:.4f} | log Learning Rate: {np.log10(current_lr):.4f}")
        # print(f"Epoch {epoch:4d} | Training Loss: {epoch_train_loss:.4f} | Validation Loss: {epoch_val_loss:.4f}")
        # print(f"Epoch {epoch:4d} | Training Loss: {loss_train.item():.4f} | Validation Loss: {loss_val.item():.4f}") ### THIS WAS THE ONE I WAS USING
    ...
    ### EARLY STOPPING ###
    if epoch_val_loss < best_val_loss:          # check early stopping condition
    ...
    if stopping_counter >= patience: # stop training if patience is exceeded
        print(f'Early stopping activated at epoch {epoch}')
        epochs = epoch # for later plots
        break
    ...
    if epoch < warmup_epochs:
        ...
        scheduler.step() # use normal scheduler after warmup, this will use step_size and gamma

model.load_state_dict(best_model_state) # load the best model before early stopping
...
# make predictions on 2024-25 data
model.eval()
